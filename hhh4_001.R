...
min_date=min(nz_covid19_loaded$dayte) # "2020-02-26"
min_doy=as.numeric(strftime(min_date, format="%j")) # 57
### Checks:-
summary(nz_covid19_loaded$seq_dayte ) # Check
dim(nz_covid19_loaded ) # 1279   11
names(nz_covid19_loaded)
levels(nz_covid19_loaded$DHB) # 20 of these
# Rename DHB names that have spaces and apostrophes:
nz_covid19<-mutate(nz_covid19_loaded, DHB = revalue(DHB, 
                                c("Bay of Plenty"     = "BayofPlenty",
...
dim(nz_covid19) # 1279   11 # Still in skinny form
#
### Retain only some vars:
nz_covid19 = nz_covid19[,c("DHB", "dayte", "seq_dayte")]
dim(nz_covid19) # 1279   3
